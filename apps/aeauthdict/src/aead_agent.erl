...
double_right_rotate(CurrentRoot, LeftChild, RightChild) ->
    ...
    NewRightChild = aead:new(CurrentRoot, #{right   => RightChild,
                                            ...
                                            balance => NewRightBalance}),
    NewLeftChild = aead:new(LeftChild, #{right   => aed_ad:left(NewRoot),
                                         balance => NewLeftBalance}),
    aead:new(NewRoot, #{left => NewLeftChild,
                        right => NewRightChild,
                        balance => ?BALANCE_0}).
    ...
    case Mod:next_direction_is_left(Key, RootNode, State) of
        {true, State2} ->
            ...
            case ChangeHappened of
                true ->
                    case {ChildHeightIncreased, aead:balance(RootNode)} of
                        {true, ?BALANCE_L} ->
                            %% need to rotate
    ...
    case {Direction, LeftChild, RightChild} of
        {D, _, ?LEAF_NODE} when D >= 0 ->
            ...
            case DeleteMax of
                true ->
                    ...
                    State2 = State#{saved_node => {some, aead:visit(RightChild)}},
                    {LeftChild, true, State2};
                false ->
        ...
        {0, ?LEAF_NODE, _} ->
            {change_key_and_value_of_min_node(RightChild,
                                              aead:key(LeftChild),
                                              aead:value(LeftChild)), true, State};
        _ ->
            ...
            case Direction of
                D when D =< 0 ->
                    ...
                    {NewLeft, ChildHeightDecreased} =
                        delete_helper(Mod, LeftChild, Direction == 0, State),
                    {NewRoot, State2} =
                        case Direction of
                            0 ->
                ...
                D when D > 0 ->
                    ...
                    {NewRight, ChildHeightDecreased} =
                        delete_helper(Mod, aead:right(RootNode), deleteMax, State),
