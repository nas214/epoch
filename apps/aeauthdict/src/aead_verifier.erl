...
key_matches_leaf(Key, Node, State) ->
    ...
    case aead:compare_keys(Key, aead:key(Node)) of
        ...
        0 ->
            {true, State};
        1 ->
            case aead:compare(Key, aead:next_leaf_key(Node)) of
                -1 ->
...
reconstruct_tree(#{starting_digest := StartingDigest,
                   ...
                   root_node_height:= RootNodeHeight} = State) ->
    try
        ...
        <<LastByte:8>> = binary:last(StartingDigest),
        ...
        State2 = parse_proof(State#{max_nodes => MaxNodes}),
        {ok, State2}
    catch
        _ : Error ->
            Error
...
digest(#{top_node := TopNode}) ->
    aead_agent:digest(TopNode).
...
parse_proof(#{proof := Proof} = State) ->
    ...
    case Nodes of
        [TopNode] ->
            ...
            SDLen = 8*size(Label),
            case maps:get(starting_digest, State2) of
                <<Label:SDLen, _>> ->
                    State2#{top_node => TopNode};
                _ ->
                    throw({error, incorrect_root_label})
            end;
        _ ->
            throw({error, unused_nodes_in_proof})
    end.
...
parse_proof(<<?LABEL_IN_PACKAGED_PROOF, Rest/binary>>, {Nodes, PreviousLeaf}, State) ->
    ...
    parse_proof(Rest2, {[aead:label_only_node(Label) | Nodes],
                        PreviousLeaf}, State);
    ...
    {Value, Rest4} =
        case ?VALUE_BITS of
            ...
            none ->
                ...
                <<V:VLength, Rest41/binary>> = Rest40,
                {V, Rest41}
        end,
    Leaf = #{key       => Key,
             visited   => false,
...
get_max_nodes({some, MaxOperations}, RootNodeHeight, MaxDeletes) ->
    ...
    RealNumOperations = case MaxOperations of
                            {some, MO} -> MO;
                            none       -> 0
                        end,
