...
-spec next_direction_is_left(aead:key(), aead:internal_node(), aed_agent:state()) ->
                                    {boolean(), aed_agent:state()}.
next_direction_is_left(_Key, _Node, #{found := true} = State) ->
    {true, State};
...
generate_proof(#{top_node     := TopNode,
                 ...
                 directions   := Directions} = State) ->
    {TopNode, PackagedTree} = pack_tree(OldTopNode),
    ResetTopNode = aead:reset_new_visited(TopNode),
    DirectionsBin = directions_to_binary(Directions),
    {<<PackagedTree/binary, ?END_OF_TREE_IN_PACKAGED_PROOF, DirectionsBin>>,
...
pack_tree(Node) ->
    {N, _, Pack} = pack_tree(Node, <<>>, false),
    {N, Pack}.

-spec pack_tree(aead:prover_node(), boolean(), packaged_tree()) ->
...
pack_tree(Node, PreviousLeafAvailable, PackagedTree) ->
    ...
    case aead:visited(Node) of
        false ->
            ...
            %% Test label size
            ?DIGEST_BITS = 8 * size(Label),
            DBits = ?DIGEST_BITS,
            {Node, true,
             <<PackagedTree/binary, ?LABEL_IN_PACKAGED_PROOF, Label:DBits/binary>>};
        true ->
            case Node of
                ?PROVER_LEAF ->
                    NodeUnvisited = aead:unvisit(Node),
                    ...
                    case PreviousLeafAvailable of
                        true ->
                            {NodeUnvisited, true,
                             <<PackagedTree, NextLeafKey:?KEY_BITS,
                               ValueBin/binary>>};
                        false ->
                            Key = aead:key(Node),
                            {NodeUnvisited, true,
                             <<PackagedTree, Key:?KEY_BITS, NextLeafKey:?KEY_BITS,
                               ValueBin/binary>>}
                    end;
                ?PROVER_INTERNAL_NODE ->
                    ...
                    {aead:new(Node, #{left    => LeftNode,
                                      right   => RightNode,
                                      visited => false}),
                     PLAR, <<PackagedTree/binary, PTL/binary, PTR/binary, Balance:8>>}
            end
    end.
...
directions_to_binary([], Acc) ->
    Acc;
directions_to_binary([H | T], Acc) ->
    directions_to_binary(T, <<H:1, Acc>>).
